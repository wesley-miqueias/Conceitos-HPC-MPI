...
{
    return internal_Status_c2f08(c_status, f08_status);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Status_c2f08(const MPI_Status *c_status, MPI_F08_status *f08_status)
{
    QMPI_Context context;
    QMPI_Status_c2f08_t *fn_ptr;
    ...
    if (MPIR_QMPI_num_tools == 0)
        return QMPI_Status_c2f08(context, 0, c_status, f08_status);

    fn_ptr = (QMPI_Status_c2f08_t *) MPIR_QMPI_first_fn_ptrs[MPI_STATUS_C2F08_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_STATUS_C2F08_T], c_status, f08_status);
}
#else /* ENABLE_QMPI */
int MPI_Status_c2f08(const MPI_Status *c_status, MPI_F08_status *f08_status)
{
