...
int QMPI_Status_f2c(QMPI_Context context, int tool_id, const MPI_Fint *f_status,
                    MPI_Status *c_status)
{
    return internal_Status_f2c(f_status, c_status);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Status_f2c(const MPI_Fint *f_status, MPI_Status *c_status)
{
    QMPI_Context context;
    QMPI_Status_f2c_t *fn_ptr;
    ...
    if (MPIR_QMPI_num_tools == 0)
        return QMPI_Status_f2c(context, 0, f_status, c_status);

    fn_ptr = (QMPI_Status_f2c_t *) MPIR_QMPI_first_fn_ptrs[MPI_STATUS_F2C_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_STATUS_F2C_T], f_status, c_status);
}
#else /* ENABLE_QMPI */
int MPI_Status_f2c(const MPI_Fint *f_status, MPI_Status *c_status)
{
