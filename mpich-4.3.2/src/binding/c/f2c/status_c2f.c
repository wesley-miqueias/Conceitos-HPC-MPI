...
int QMPI_Status_c2f(QMPI_Context context, int tool_id, const MPI_Status *c_status,
                    MPI_Fint *f_status)
{
    return internal_Status_c2f(c_status, f_status);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Status_c2f(const MPI_Status *c_status, MPI_Fint *f_status)
{
    QMPI_Context context;
    QMPI_Status_c2f_t *fn_ptr;
    ...
    if (MPIR_QMPI_num_tools == 0)
        return QMPI_Status_c2f(context, 0, c_status, f_status);

    fn_ptr = (QMPI_Status_c2f_t *) MPIR_QMPI_first_fn_ptrs[MPI_STATUS_C2F_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_STATUS_C2F_T], c_status, f_status);
}
#else /* ENABLE_QMPI */
int MPI_Status_c2f(const MPI_Status *c_status, MPI_Fint *f_status)
{
