...
{
    return internal_Remove_error_string(errorcode);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Remove_error_string(int errorcode)
{
    QMPI_Context context;
    QMPI_Remove_error_string_t *fn_ptr;

    context.storage_stack = NULL;

    if (MPIR_QMPI_num_tools == 0)
        return QMPI_Remove_error_string(context, 0, errorcode);

    fn_ptr = (QMPI_Remove_error_string_t *) MPIR_QMPI_first_fn_ptrs[MPI_REMOVE_ERROR_STRING_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_REMOVE_ERROR_STRING_T], errorcode);
}
#else /* ENABLE_QMPI */
int MPI_Remove_error_string(int errorcode)
{
