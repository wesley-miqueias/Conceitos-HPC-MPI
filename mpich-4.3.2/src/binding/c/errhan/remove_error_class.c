...
{
    return internal_Remove_error_class(errorclass);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Remove_error_class(int errorclass)
{
    QMPI_Context context;
    QMPI_Remove_error_class_t *fn_ptr;

    context.storage_stack = NULL;

    if (MPIR_QMPI_num_tools == 0)
        return QMPI_Remove_error_class(context, 0, errorclass);

    fn_ptr = (QMPI_Remove_error_class_t *) MPIR_QMPI_first_fn_ptrs[MPI_REMOVE_ERROR_CLASS_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_REMOVE_ERROR_CLASS_T], errorclass);
}
#else /* ENABLE_QMPI */
int MPI_Remove_error_class(int errorclass)
{
