...
{
    return internal_Remove_error_code(errorcode);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Remove_error_code(int errorcode)
{
    QMPI_Context context;
    QMPI_Remove_error_code_t *fn_ptr;

    context.storage_stack = NULL;

    if (MPIR_QMPI_num_tools == 0)
        return QMPI_Remove_error_code(context, 0, errorcode);

    fn_ptr = (QMPI_Remove_error_code_t *) MPIR_QMPI_first_fn_ptrs[MPI_REMOVE_ERROR_CODE_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_REMOVE_ERROR_CODE_T], errorcode);
}
#else /* ENABLE_QMPI */
int MPI_Remove_error_code(int errorcode)
{
