...
static int internal_Status_get_error(const MPI_Status *status, int *error)
{
...
{
    return internal_Status_get_error(status, error);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Status_get_error(const MPI_Status *status, int *error)
{
    QMPI_Context context;
    QMPI_Status_get_error_t *fn_ptr;
    ...
    if (MPIR_QMPI_num_tools == 0)
        return QMPI_Status_get_error(context, 0, status, error);

    fn_ptr = (QMPI_Status_get_error_t *) MPIR_QMPI_first_fn_ptrs[MPI_STATUS_GET_ERROR_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_STATUS_GET_ERROR_T], status, error);
}
#else /* ENABLE_QMPI */
int MPI_Status_get_error(const MPI_Status *status, int *error)
{
