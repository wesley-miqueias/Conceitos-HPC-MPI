...
int QMPI_Session_iflush_buffer(QMPI_Context context, int tool_id, MPI_Session session,
                               MPI_Request *request)
{
    return internal_Session_iflush_buffer(session, request);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Session_iflush_buffer(MPI_Session session, MPI_Request *request)
{
    QMPI_Context context;
    QMPI_Session_iflush_buffer_t *fn_ptr;

    context.storage_stack = NULL;

    if (MPIR_QMPI_num_tools == 0)
        return QMPI_Session_iflush_buffer(context, 0, session, request);

    fn_ptr = (QMPI_Session_iflush_buffer_t *) MPIR_QMPI_first_fn_ptrs[MPI_SESSION_IFLUSH_BUFFER_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_SESSION_IFLUSH_BUFFER_T], session,
            request);
}
#else /* ENABLE_QMPI */
int MPI_Session_iflush_buffer(MPI_Session session, MPI_Request *request)
