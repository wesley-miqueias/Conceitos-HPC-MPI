...
{
    return internal_Comm_iflush_buffer(comm, request);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Comm_iflush_buffer(MPI_Comm comm, MPI_Request *request)
{
    QMPI_Context context;
    QMPI_Comm_iflush_buffer_t *fn_ptr;

    context.storage_stack = NULL;

    if (MPIR_QMPI_num_tools == 0)
        return QMPI_Comm_iflush_buffer(context, 0, comm, request);

    fn_ptr = (QMPI_Comm_iflush_buffer_t *) MPIR_QMPI_first_fn_ptrs[MPI_COMM_IFLUSH_BUFFER_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_COMM_IFLUSH_BUFFER_T], comm, request);
}
#else /* ENABLE_QMPI */
int MPI_Comm_iflush_buffer(MPI_Comm comm, MPI_Request *request)
{
