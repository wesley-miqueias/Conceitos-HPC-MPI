...
int QMPI_Comm_detach_buffer(QMPI_Context context, int tool_id, MPI_Comm comm, void *buffer_addr,
                            int *size)
{
    return internal_Comm_detach_buffer(comm, buffer_addr, size);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Comm_detach_buffer(MPI_Comm comm, void *buffer_addr, int *size)
{
    QMPI_Context context;
    QMPI_Comm_detach_buffer_t *fn_ptr;

    context.storage_stack = NULL;

    if (MPIR_QMPI_num_tools == 0)
        return QMPI_Comm_detach_buffer(context, 0, comm, buffer_addr, size);

    fn_ptr = (QMPI_Comm_detach_buffer_t *) MPIR_QMPI_first_fn_ptrs[MPI_COMM_DETACH_BUFFER_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_COMM_DETACH_BUFFER_T], comm,
            buffer_addr, size);
}
#else /* ENABLE_QMPI */
int MPI_Comm_detach_buffer(MPI_Comm comm, void *buffer_addr, int *size)
...
int QMPI_Comm_detach_buffer_c(QMPI_Context context, int tool_id, MPI_Comm comm, void *buffer_addr,
                              MPI_Count *size)
{
    return internal_Comm_detach_buffer_c(comm, buffer_addr, size);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Comm_detach_buffer_c(MPI_Comm comm, void *buffer_addr, MPI_Count *size)
{
    QMPI_Context context;
    QMPI_Comm_detach_buffer_c_t *fn_ptr;

    context.storage_stack = NULL;

    if (MPIR_QMPI_num_tools == 0)
        return QMPI_Comm_detach_buffer_c(context, 0, comm, buffer_addr, size);

    fn_ptr = (QMPI_Comm_detach_buffer_c_t *) MPIR_QMPI_first_fn_ptrs[MPI_COMM_DETACH_BUFFER_C_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_COMM_DETACH_BUFFER_C_T], comm,
            buffer_addr, size);
}
#else /* ENABLE_QMPI */
int MPI_Comm_detach_buffer_c(MPI_Comm comm, void *buffer_addr, MPI_Count *size)
