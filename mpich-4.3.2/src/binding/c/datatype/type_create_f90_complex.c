...
{
    return internal_Type_create_f90_complex(p, r, newtype);
}
#endif /* MPICH_MPI_FROM_PMPI */
int MPI_Type_create_f90_complex(int p, int r, MPI_Datatype *newtype)
{
    ...
    if (MPIR_QMPI_num_tools == 0)
        return QMPI_Type_create_f90_complex(context, 0, p, r, newtype);

    fn_ptr = (QMPI_Type_create_f90_complex_t *) MPIR_QMPI_first_fn_ptrs[MPI_TYPE_CREATE_F90_COMPLEX_T];

    return (*fn_ptr) (context, MPIR_QMPI_first_tool_ids[MPI_TYPE_CREATE_F90_COMPLEX_T], p, r,
            newtype);
}
#else /* ENABLE_QMPI */
int MPI_Type_create_f90_complex(int p, int r, MPI_Datatype *newtype)
